...
export const AuditProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Load audits from localStorage on component mount
    const savedAudits = localStorage.getItem('5s_audit_data');
    if (savedAudits) {
  ...
  const saveAuditsToStorage = (updatedAudits: Audit[]) => {
    localStorage.setItem('5s_audit_data', JSON.stringify(updatedAudits));
    setAudits(updatedAudits);
  };

  const createAudit = (auditData: Omit<Audit, 'id' | 'createdAt' | 'updatedAt'>): string => {
    if (!user) throw new Error('User must be authenticated to create audit');
